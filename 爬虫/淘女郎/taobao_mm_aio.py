...
import json, urllib2, time, aiohttp, asyncio
import re
...
class TaoGirl():
    ...
    async def parse_data(self, url, page_max):
        while self.c_page <= page_max:
            doc = pq(await self.fetch(url))
    ...
    async def download(self, page_max=99999999, row_max=999999999999):
...
class TaoGirl_model_info(TaoGirl):
    def save_data(self, data, col_name):
        ...
        bmi = float(data.get('db_document').get('weight')[:-2]) / float(
            data.get('db_document').get('height')[:-2]) ** 2 / 10000
        doc = pq(url)
