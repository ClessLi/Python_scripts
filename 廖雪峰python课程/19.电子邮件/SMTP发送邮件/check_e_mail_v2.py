...
class e_mail():
    def __init__(self,address):
        ...
        self.check_flag=self.check()
    def check(self):
        if self.match(self.rule1,self.address):
            self.name = self.rule1.match(self.address).group(1)
            return True
            #print '%s is an e-mail address.' % address
        elif self.match(self.rule2,self.address):
            self.name = self.rule2.match(self.address).group(1)
...
if __name__ == '__main__':
    ...
    print ('%s is an e-mail address and name is %s.' % (r.address, r.name) if r.check_flag else '%s is not an e-mail address.' % r.address)
